...
export const AboutUsFirstSection = ({ onLinkClick }) => {
  ...
  const handledarbandi = () => {
    navigate("/darbandi");
  };
  const openPdf = () => {
    const pdfUrl =
      "https://docs.google.com/spreadsheets/d/1AHpgJO-5IF0BKKPMlQAOpIeuVwX87SAh1vXHqLKDHX0/edit?gid=0#gid=0"; // Replace with your PDF URL
  ...
  return (
    <div className="flex items-center flex-col mb-6 w-full ">
      <div
        className={`${
          isMobile ? "text-[30px] h-[30vh]" : "text-[60px] h-[30vh]"
        } w-full  font-bold text-white  flex justify-center items-center z-1`}
      >
        {t("about-us")}
      </div>
      <div className={`${isMobile ? "w-[95%]" : "w-[80%] "}  rounded-md `}>
        <div
          className={`${
            isMobile ? "gap-7" : " gap-24"
          } w-8/10 h-8/10 flex flex-wrap items-center justify-center  rounded-md `}
        >
          <InstanceFirstSection
            bgImage={introImg}
          ...
          <InstanceFirstSection
            bgImage={histoImg}
            name={"historical-background-tab"}
            onLinkClick={onLinkClick}
          />
          <InstanceFirstSection
            bgImage={rightImg}
            name={"rights-and-duties-tab"}
            onLinkClick={onLinkClick}
          />
          <InstanceFirstSection
            bgImage={orgImg}
            name={"organizational-structure-tab"}
            onLinkClick={() => {
              handledarbandi();
            }}
          />
