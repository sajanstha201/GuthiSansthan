...
export const HomePage = () => {
  ...
  return (
    <div style={{ height: `${isMobile ? 'calc(100vh - 80px)' : 'calc(100vh - 100px)'}` }}>
      <video autoPlay loop muted className="top-0 video-background absolute inset-0 w-full h-full object-cover -z-30">
      ...
      <div className="flex flex-col items-center justify-center h-full relative overflow-hidden">
        ...
        <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'}  gap-4 px-5 w-full`}>
          ...
          >
            {t('sign-in')} {/* Access translated text using t('key') */}
          </Link>
        </div>
        <HomePageFooter/>
      </div>
