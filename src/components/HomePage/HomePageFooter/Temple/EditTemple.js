...
export const EditTemple = ({ name, detail, location, templeDetail, templeId, fetchAllTemple, img, qr }) => {
    ...
    const saveEdits = async () => {
        try {
            ...
            await axios.patch(baseUrl + templeDetail.dynamicUrl + templeId, formData, {
                headers: {
            ...
            <div className="w-full py-2 bg-white/40 flex flex-col items-center px-10">
                ...
                <div className='flex flex-col flex-wrap py-1 border-y-2 w-1/2 border-b-zinc-700/5  gap-2 items-center'>
                    <div className='flex flex-col items-center'>
                        <label className='font-semibold text-lg mb-2 text-white' htmlFor="temple-edit-image">Upload Image:</label>
                        <label
                        ...
                        >
                            {!image ? (
                                ...
                                <img src={image} className='w-full h-full rounded-md' alt="Temple" />
                            )}
                        </label>
                        <input
                            type='file'
                    ...
                    <div className='flex flex-col items-center'>
                        <label className='font-semibold text-lg mb-2 text-white'>Upload QR Code:</label>
                        <label
                            ...
                            style={{ backgroundImage: `url(${qrCode})`, backgroundSize: 'cover' }}
                        >
                            {!qrCode ? (
                                <FontAwesomeIcon icon={faClose} className='text-white text-2xl' />
                            ) : (
                                <img src={qrCode} className='w-full h-full rounded-md' alt="QR Code" />
                            )}
                        </label>
                        <input
                            type='file'
