    ...
    return (
        <div className={`${isMobile ? 'h-[80px] flex-wrap w-full' : 'flex-row px-20 h-[100px]'} flex w-full bg-black/20 top-0 justify-between items-center p-2`}>
            <Link to='/'
                ...
                <img className={`${isMobile ? 'h-[30px]' : 'h-[80px]'}`} src={nepalLogo} alt="Nepal Logo" />
                <EditImage imageId={globalDetail['guthi-sansthan-logo'].id} url={baseUrl + globalDetail.url} setNewImage={setNewGuthiSansthanLogo} isActualUploadedSame={globalDetail['guthi-sansthan-logo'].imgSrc === globalDetail['guthi-sansthan-logo'].actualImgSrc}>
                    <img className={`${isMobile ? 'h-[30px] pr-4' : 'h-[80px] pr-10'} backdrop-blur-md bg-neutral-200/40 rounded-full shadow-lg`} src={globalDetail['guthi-sansthan-logo'].imgSrc} alt="Guthi Sansthan Logo" />
                </EditImage>
            </Link>
            {!isMobile &&
                <>
            ...
            <div className={`${isMobile ? 'gap-1 w-[50%]' : 'gap-7 w-[30%]'} relative flex-row flex h-full items-center justify-start px-2`}>
                    ...
                    <div className="relative bg-gray-300/70 rounded-full flex flex-row items-center justify-center">
                        <div className={`h-[30px] w-[30px] md:h-[60px] md:w-[60px] cursor-pointer transition-all rounded-md p-1 items-center flex justify-center`}
                            ...
                            <div className="z-30 p-2 overflow-hidden h-full w-full items-center flex justify-center">
                                {selectLanguage === 'nepali' && <img src={globalDetail['lng-logo']['nepali']} alt="Nepali Language" />}
                                {selectLanguage === 'newari' && <img src={globalDetail['lng-logo']['newari']} alt="Newari Language" />}
                                {selectLanguage === 'english' && <img src={globalDetail['lng-logo']['english']} alt="English Language" />}
                                {selectLanguage === 'mithila' && <img src={globalDetail['lng-logo']['mithila']} alt="Mithila Language" />}
                            </div>
                ...
                {token && <div ref={dropdownRef} className="absolute right-4 inline-block text-left text-white hover:text-blue-500">
