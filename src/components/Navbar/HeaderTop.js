...
export const HeaderTop=()=>{
    ...
    return(
        <div className={`${isMobile?' h-[50px]':'flex-row  px-20 h-[100px]'} flex w-screen justify-between items-center p-2 `} >
            ...
            {loc.pathname!=='/'&&!isMobile&&<div className={`${isMobile?'':''} w-[30%] flex items-center justify-center`}>
                <HeaderButtom/>
            </div>}
            <div className={`${isMobile?'gap-2 w-[50%]':' gap-7 w-[40%]'}  flex-row flex h-full items-center justify-center `}>
                <div className={`${isMobile?' h-[100px] gap-2':' h-full gap-7'} w-[50%] flex items-center justify-end `}>
                    <div 
                ...
                <div className={`${isMobile?'text-[10px] h-[25px] w-[30%]':'h-[60%] w-[10%] px-5 py-2'}  bg-red-600 text-white flex items-center justify-center mx-2 rounded-full hover:bg-red-700 cursor-pointer shadow-sm font-bold`}>
                    {t('donate')}
                </div>
            </div>
        </div>
